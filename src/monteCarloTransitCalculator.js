...
export class MonteCarloTransitCalculator {
    ...
    constructor(star, numSamples){
        // do not use the setter as we do not want to calculate drawPoints twice
        this._star = star;
    ...
    set numSamples(numSamples) {
        // theta does not need to be recalculated when the star is updated
        self.theta = Array.from({ length: numSamples }, () => Math.random() * 2 * Math.PI);
    ...
    drawPoints(numSamples) {
        console.log("Drawing points for Monte Carlo simulation with numSamples:", numSamples);
        const r = Array.from({ length: numSamples }, () => Math.sqrt(Math.random()) * this.star._R);
        this.ypoints = r.map((rVal, i) => rVal * Math.cos(self.theta[i]));
        this.zpoints = r.map((rVal, i) => rVal * Math.sin(self.theta[i]));
    }
